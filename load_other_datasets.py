...
def load_cornell_dataset(path='../data/raw_data/', dataset = 'amazon', 
    ...
    features[np.arange(num_nodes), labels - 1] = 1#[node*labels]
    if feature_dim is not None:
        num_row, num_col = features.shape
        zero_col = np.zeros((num_row, feature_dim - num_col), dtype = features.dtype)
        features = np.hstack((features, zero_col))#[node*labels, node*zeros]

    features = np.random.normal(features, feature_noise, features.shape)#[node*labels, node*zeros]
